<!-- source: action_and_event_data_generation/ext_replicator-object/mutable_attribute.html | title: Mutable Attribute — Isaac Sim Documentation -->

# Mutable Attribute
A value in the description file is a mutable attribute if it is a dictionary that has a key `distribution_type`, or a string that contains macros (`$[...]`). A mutable attribute does not have to be part of a mutable; you can have standalone mutable attributes.
...
Camera frustum
A `camera_frustum` attribute is specially used for sampling a value for the translate operator (Refer to Transformation ). It samples a position in a view frustum defined by `camera_parameters`, which is the same as in Camera .
...
\[distance = \frac{distanceMin \cdot distanceMax}{distanceMin + (distanceMax - distanceMin) \cdot randomUnit}\]in which \(randomUnit\) is uniformly sampled within `[0,1]`.
Harmonized
A `harmonized` attribute defines an attribute that retrieves its value from a Harmonizer after harmonization . More details can be found in the harmonization example .
