<!-- source: action_and_event_data_generation/ext_replicator-object/transformation.html | title: Transformation — Isaac Sim Documentation -->
...
Define these entities in your descriptions. A camera, with camera parameters defined as described in Camera ; A Dome light so that you can see things; and Oro, which is a Geometry . The observatory is only conceptual, you don’t need to see it.
...
- 1000
```
Then rotate the scope around the X-axis by 30 degrees. This applies a `rotateX` operator, during the original translate.
...
type: camera
```
The created scene with embedded interface :
The visualization using the distribution visualizer :
