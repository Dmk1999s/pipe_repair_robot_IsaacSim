<!-- source: action_and_event_data_generation/tutorial_replicator_object.html | title: Object Simulation and Synthetic Data Generation — Isaac Sim Documentation -->

# Object Simulation and Synthetic Data Generation
The `isaacsim.replicator.object` (IRO) is a no-code-change-required extension that generates synthetic data for model training that can be used on a range of tasks from retail object detection to robotics. The extension can be run from the UI or the `isaac-sim` container.
...
- In Omniverse Isaac Sim 5.0, enable `isaacsim.replicator.object` from Windows > Extension manager.

- Search for `isaacsim.replicator.object`, and click on the green capsule icon to enable the extension.

- If the extension is successfully enabled, Object Detection SDG panel will be available at the top right, and Tools > Action and Event Data Generation will have options Object Detection SDG and Distribution Visualizer.
If not, disable and enable the extension again. The Object Detection SDG panel is turned on by default, and you can turn it off or on again by Tools > Action and Event Data Generation > Object Detection SDG.
[image: ../_images/isim_5.0_replicator_ext-isaacsim.replicator.object-0.4.2_viewport_ui_main_panel.webp]
...
- Under `PATH_TO_EXTENSION/isaacsim/replicator/object/configs` there are many description files in YAML format. It’s recommended that you start with `demo_kaleidoscope.yaml`.
...
- replace `PATH_TO_USD` with a folder containing USD files.
...
- Select `demo_kaleidoscope` from the dropdown box; `demo_kaleidoscope` will appear in the Description File text box. You can also use the full absolute path `PATH_TO_EXTENSION/isaacsim/replicator/object/configs/demo_kaleidoscope.yaml` to load a description file.

- Click Simulate to start the simulation. The progress bar will show the simulation progress.

In the above and following content, `PATH_TO_EXTENSION` varies, for Isaac on Windows it is something like `C:\isaacsim\extscache\isaacsim.replicator.object-0.4.x\omni\replicator\object\configs\demo_kaleidoscope.yaml`, while for Isaac on Linux it is something like `~/isaacsim/extscache/isaacsim.replicator.object-0.4.x/isaacsim/replicator/object/configs/demo_kaleidoscope.yaml`
...
```
bash isaac-sim.sh --no-window --enable isaacsim.replicator.object --allow-root --/log/file=/tmp/isaacsim.replicator.object.log --/log/level=warn --/windowless=True --/config/file=PATH_TO_EXTENSION/isaacsim/replicator/object/configs/demo_kaleidoscope.yaml
```
...
[image: ../_images/isim_5.0_replicator_ext-isaacsim.replicator.object-0.4.2_viewport_ui_embedded_interface.webp]To preview physically, click on the triangular Play button on the left column of widgets.
...
## Concepts
Description FileThe description file is a YAML file that has a main key named `isaacsim.replicator.object`.
...
seed: 0
```
Simulation WorkflowEvery time a simulation is launched, an initialization stage happens in the beginning, and a per-frame simulation stage happens every frame.
...
The first step is to set the description files. Turn on the extension manager, search for `isaacsim.replicator.object`, and click on the Open Extension Folder button, as shown below.
Note
If `isaacsim.replicator.object` is not enabled, click on the capsule icon to enable it.
[image: ../_images/isim_5.0_replicator_ext-isaacsim.replicator.object-0.4.2_viewport_open_extension_folder.webp]In the folder, go to `PATH_TO_EXTENSION/isaacsim/replicator/object/configs`. On Windows, the folder is opened after the Open Extension Folder button is clicked. On Linux, it can bring up the browser with the URL as `file://EXTENSION_PATH`, in this case, navigate to `EXTENSION_PATH` using the command line or `xdg-open`.
...
[image: ../_images/isim_5.0_replicator_ext-isaacsim.replicator.object-0.4.2_viewport_ui_demo_table.webp]
...
[image: ../_images/isim_5.0_replicator_ext-isaacsim.replicator.object-0.4.2_viewport_drag_and_drop.webp]Adjust the position of the object about to be scattered onto the table, for a reasonable range of its position.
[image: ../_images/isim_5.0_replicator_ext-isaacsim.replicator.object-0.4.2_viewport_gauge.webp]From observation, from `(-13, 100, -70)` to `(13, 100, 70)` is a reasonable range for the position of the box. Compose a description file as follows:
...
[image: ../_images/isim_5.0_replicator_ext-isaacsim.replicator.object-0.4.2_viewport_toggle_visibility.webp]Note
