<!-- source: isaac_lab_tutorials/tutorial_policy_deployment.html | title: Deploying Policies in Isaac Sim — Isaac Sim Documentation -->
...
- Open the example using Robotics Examples > POLICY > Humanoid.
...
- Open the example using Robotics Examples > POLICY > Quadruped.
...
Note
See isaac sim policy example extension document for standalone example workflow and the policy files used in the examples.
...
```
Note
The trained policy files used in the examples are available to download here .
...
### Forward
This function must be overload by the inherited class and is called every physics step to generate control action for the robot. For example, the code snippet below creates the controls for the H1 flat terrain policy.
...
#### Physics Scene
Physics scene describes the time stepping with `Time Steps Per Second (Hz)`, so take the inverse of the `dt` parameter in the `env.yaml` and set this correctly. Also match the physics scene properties with the physx section of the `env.yaml` file.
