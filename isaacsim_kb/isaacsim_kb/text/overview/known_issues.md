<!-- source: overview/known_issues.html | title: Known Issues — Isaac Sim Documentation -->
...
- Dragging and dropping an asset with default values in OmniGraph nodes should be saved with scene and reloaded before hitting `Play` to make sure all values are correctly set.
...
[image: ../_images/isim_4.5_full_tut_gui_filepicker_known_issue_with_s3_assets.png]
...
- Replicator synthetic data generation may require more subframes to be rendered for scenes with significant changes (e.g. moving objects or changing lighting conditions). See RT Subframes Parameter and subframes examples for more information.
...
- Python errors related to `tkinter` like the following indicate the user is attempting to use `tkinter` with the Python distribution shipped with Isaac Sim. This is not supported.
