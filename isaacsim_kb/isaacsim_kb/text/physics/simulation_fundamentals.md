<!-- source: physics/simulation_fundamentals.html | title: Physics Simulation Fundamentals — Isaac Sim Documentation -->
...
- Add a Torus mesh in the place of the platform at `Z=3.0` and scale it to [5.0, 5.0, 5.0].
...
[image: ../_images/isaac_sim_joint_pose.png]Note that when creating a joint through the UI, the joint’s frames are set to match the pose of the second rigid body selected for the creation.
...
- Click on the selected physics element.

- “Add” > Physics > Residual Reporting.

- Verify that you can see the Residual plot over time on the Simulation Data Visualizer: (eye icon on viewport) > Show by Type > Physics > Simulation Data Visualizer.
