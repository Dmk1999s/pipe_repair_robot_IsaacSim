<!-- source: py/docs/overview/guidelines.html | title: Coding Style Guidelines — Isaac Sim -->

# Coding Style Guidelines

## C/C++
This guideline is a short adaptation of the Coding Style Guidelines for Omniverse Carbonite SDK, the foundational software layer for Omniverse applications,microservices, tools, plugins, Connectors, and SDKs.

- Visit the Carbonite C++17 and Beyond Recommendations section for using C++17 features.

- Visit the Carbonite Higher-level Concepts section for using Carbonite for thread-safety , assertions , callbacks , exceptions , logging , and other topics.
...
Hint
Consult the antonym list when naming symmetric functions.
...
- Use stateful names for boolean variables (e.g.: `enabled`, `m_initialized`, `g_cached`) and leave questions for methods (e.g.: `isXxxx()` and `hasXxxx()`).
...
- Avoid conjunctions and sentences in names as much as possible. E.g.: Use `Count` at the end of a name for the number of items.
...
- Do not include `Windows.h` in header files as it is monolithic and pollutes the global environment for Windows. Instead, a much slimmer CarbWindows.h exists to declare only what is needed by Carbonite. Refer to the example to see how to include it.

- Local includes use the path-relative include format.

- Includes of files that are not local to the code (or are pulled in via package) use the search path format. Isaac Sim source files (under `plugins/` and `source/`) may also use search-path format for public headers (under `include/`).

- If you need to associate a comment with an include put the comment on the same line as the include statement, otherwise clang-format will not move the chunk of code. Like this:
...
- For public header files, a `details` (internal) namespace should be used to declare implementation as private and subject to change, as well as signal to external users that the functions, types, etc. in the `details` namespace should not be called.
...
- When adding `#if` pre-processor blocks to support multiple platforms, the block must end with an `#else` clause containing the `CARB_UNSUPPORTED_PLATFORM()` macro. An exception to this is when the `#else` block uses entirely C++ standard code; this sometimes happens in the case of platform-specific optimizations. You may not make assumptions about what features future platforms may have, aside from what’s in the C++ standard; all platform-specific code must have the associated platform specifically stated.
...
- Macros that do not have universal appeal (i.e. are only intended to be used within a single header file) shall be prefixed with `ISAACSIMLOCAL_` and `#undef`’d at the end of the file.
