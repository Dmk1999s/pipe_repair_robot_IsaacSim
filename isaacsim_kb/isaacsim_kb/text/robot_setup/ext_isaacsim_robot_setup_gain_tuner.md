<!-- source: robot_setup/ext_isaacsim_robot_setup_gain_tuner.html | title: Gain Tuner Extension — Isaac Sim Documentation -->
...
This page provides the explanation of the function behind the Gain Tuner. For more detail about the step-by-step usage of the Gain Tuner, refer to Tutorial 11: Tuning Joint Drive Gains tutorial page.
...
\[\tau = \text{stiffness} * (q - q_{\text{target}}) + \text{damping} * (\dot{q} - \dot{q}_{\text{target}})\]where \(q\) and \(\dot{q}\) are the joint position and velocity, respectively. When \(\dot{q}_{target} = 0\), the system reduces to a conventional PD controller on the joint position.
...
\[Effort = K_p * (Position_{Desired} - Position_{Current}) + K_d * (Velocity_{Desired} - Velocity_{Current})\]Where \(K_p\) is the Stiffness and \(K_d\) is the Damping.
...
\[ \begin{align}\begin{aligned}\omega_n = \sqrt{\frac{K_p}{m}}\\\zeta = \frac{K_d}{2 m \omega_n}\end{aligned}\end{align} \]Where \(\omega_n\) is the natural frequency and \(\zeta\) is the damping ratio, and \(m\) is the computed joint inertia based on the mass of the robot at both sides of the joint. The damping ratio is such that \(\zeta = 1.0\) is a critically damped system, \(\zeta < 1.0\) is underdamped, and \(\zeta > 1.0\) is overdamped.
...
[image: ../_images/isim_5.0_full_ref_gui_gains_tuner_test_settings.png]

## Further Learning

- The Tutorial 11: Tuning Joint Drive Gains tutorial for detailed instructions on how to use the Gain Tuner.

- Use Adding a Controller , if you want to send direct effort commands and implement your own joint drives.
