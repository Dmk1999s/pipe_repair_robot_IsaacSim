<!-- source: robot_setup/robot_wizard_tutorials.html | title: Robot Wizard Tutorial — Isaac Sim Documentation -->

# Robot Wizard Tutorial
This tutorial is a step-by-step guide for using the Robot Wizard to create a mock robot that contains a fixed base, a prismatic joint, and a revolute joint. Load the prepared file for this tutorial onto stage. Go to `Isaac/Samples/Rigging/RobotWizard/` and open the original file for `raw_blocks.usd`.
For more in-depth explanation about the Wizard, refer to Robot Wizard [Beta] .
...
- Click on the Create New Joint button to add a new joint.

- Make three joints for this robot. Select Create to add the first two, and Create & Close to add the third.

[TABLE]
Joint Name | Joint Type | Axis | Parent Link | Child Link | Driver Type
fixed_joint | Fixed | — | — | <robot_name>/link1 | —
slider_joint | Prismatic | X | <robot_name>/link1 | <robot_name>/link2 | force
rotate_joint | Revolute | Z | <robot_name>/link2 | <robot_name>/link3 | force
[/TABLE]

- Update the joint parameters for the `slider_joint ``and ``rotate_joint`. Click on the joint name in the table to open the joint settings for the selected joint.
...
- Click Save Robot to save the robot and move on to the next page.
...
[image: ../_images/isim_5.1_full_tut_gui_wizard_final.webp]
