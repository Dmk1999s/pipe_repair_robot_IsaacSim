<!-- source: robot_setup_tutorials/tutorial_intro_assemble_robot.html | title: Tutorial 2: Assemble a Simple Robot — Isaac Sim Documentation -->
...
- Complete Tutorial 1: Stage Setup prior to beginning this tutorial.
...
- Create a cube clicking Create > Shape > Cube in the top menu bar. You should see the cube and the Movegizmo (the red, blue, and green arrows) appear in the viewport window
...
[image: ../_images/isim_4.5_base_ref_gui_simple_objs_body.png]

## Adding Physics Properties
The cubes and cylinders added so far are strictly visual prims, with no physics or collision properties attached to them. When you start the simulation by pressing Play and gravity is applied, these objects do not move because they are unaffected by physics.
...
- To remove APIs, click on the `X` to delete the section.

Hint
Dynamic objects can only select from Convex Hull, Convex Decomposition, Sphere Approximation, SDF mesh (GPU backend only) for collision shapes. Triangle mesh collision shapes are only available for static objects.
[image: ../_images/isim_4.5_base_ref_gui_simple_objs_physics.webp]
...
[image: ../_images/isim_4.5_base_ref_gui_collision.png]
...
- Select Rigid Body Material in the popup box. A new `PhysicsMaterial` appears on the stage tree.
...
[image: ../_images/isim_4.5_base_ref_gui_materials.png]
...
[image: ../_images/isim_4.5_base_ref_gui_toggle_spotlight.png]To change the color of the object, create a different material and then assign it to the objects, just like with the physics materials. For example, create two different materials, one for the body of the car and one for the wheels.
...
[image: ../_images/isim_4.5_base_ref_gui_new_materials.png]
...
- Go to Tutorial 3: Articulate a Basic Robot to learn how to turn these geometries into a moving car.
