<!-- source: ros2_tutorials/tutorial_ros2_launch.html | title: ROS 2 Launch — Isaac Sim Documentation -->
...
- This tutorial requires the `carter_navigation`, `isaac_ros_navigation_goal`, and `isaacsim` ROS 2 packages that are provided as part of your NVIDIA Isaac Sim download. These ROS 2 packages are located inside the appropriate `ros2_ws`. They contain the required launch files, navigation parameters, and robot model. Complete ROS 2 Installation , make sure the ROS 2 workspace environment is setup correctly.
...
- version: Specify the version of Isaac Sim to use. Isaac Sim will be run from default install root folder for the specified version. Leave empty to use latest version of Isaac Sim. [default_value = “5.1.0”]
...
Note
As of Isaac Sim 5.1 only Python 3.11 is supported. Therefore `use_internal_libs` (compiled with Python 3.11) are now set to true by default. If your own ROS installation is built with Python 3.11, you can set `use_internal_libs` to false.
...
- ros_distro: Provide ROS version to use. Only Humble is supported. [default_value = “humble”]
