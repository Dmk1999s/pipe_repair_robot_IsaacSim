<!-- source: ros2_tutorials/tutorial_ros2_tf.html | title: ROS2 Transform Trees and Odometry — Isaac Sim Documentation -->
...
- Ensure the articulation root for your imported Turtlebot3 robot is `/World/turtlebot3_burger`. Otherwise, remove the articulation root from `/World/turtlebot3_burger/base_footprint` and add it for `/World/turtlebot3_burger`. Follow the steps in Articulation Transforms section to change the articulation root of the Turtlebot3 robot.
...
- To begin, enable the Isaac Sim’s transform viewer extension using the Extension Manager by searching for `isaacsim.ros2.tf_viewer`.
