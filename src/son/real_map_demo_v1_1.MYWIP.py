...
# 🌐 pipe_comm 규약 브리지 — web_panel 이 보는 drive_state/odom/imu/영상/사건을
#    낸다. 발행자는 규약 옆에 있다: `src/dongmin/isaac_bridge/`.
#    접붙임 지점은 `src/dongmin/pipe_comm/ROS2_웹연동_이식가이드.md` 의 5곳
#    (기동 / 매 스텝 / 10Hz / 상태 전이 / 지령)을 그대로 따랐다.
# 🔑 **영상은 카메라가 있을 때만 나간다.** 이 연습장은 카메라를 기본으로 만들지
#    않으므로(위 주석) `NAV=vision` 이거나 `--cam` 일 때만 웹 Camera 화면이
#    찬다. 그 밖에는 비어 있는 것이 정상이다.
# 🌐 `--ros` 면 **후방 카메라도 만든다** — 웹이 복귀·탈출 중에 가는 방향을 봐야
#    하기 때문이다. 발행은 한 번에 한 대(전진 front / 후진 rear).
#    토치 카메라는 없다(이 파일에는 용접 시퀀스가 없다).
# 🚨 영상용 annotator 를 **새로 붙이지 않는다** — 판정이 보는 바로 그 프레임을
#    그대로 발행한다(이식가이드 §1). 새로 붙이면 같은 render product 를 두 번
#    읽어 낭비이고, 무엇보다 로그와 화면이 다른 그림이 될 수 있다.
ROS = "--ros" in sys.argv
# 토픽 앞에 붙는 이름. 🔑 실전 맵은 **한 번에 한 층**만 띄우므로 로봇도 한
#    대다 — 그래서 코스 이름(floor1)이 아니라 규약 기본값 `robot` 을 쓴다.
#    web_panel 의 기본값(`-p ns:=robot`)과 맞춰 두면 웹이 그냥 붙는다.
...
_KNOWN = {"--headless", "--hold", "--glass", "--course",
          "--steps", "--reverse", "--cam", "--ros", "--wait"}
# 🔑 **서서 기다린다** — 안착만 하고 웹의 ▶ 시작을 받을 때까지 안 간다.
#    층마다 프로세스를 따로 띄워 **두 로봇을 동시에 출발**시킬 때 쓴다
#    (기동 시간이 제각각이라, 안 세워 두면 먼저 뜬 쪽이 혼자 출발한다).
#    관제의 "▶ 전체 시작"(robot:all)이 둘을 같은 순간에 푼다.
WAIT_START = "--wait" in sys.argv
...
# 층별 z 오프셋(map mm) — 위 COURSES 변환에 들어간 그 값이다. 🔑 **맵 배치를
# 아는 것은 이 파일뿐이라** 웹으로 보낼 CAD 메시도 여기서 굽는다(`--ros`).
# 받는 쪽이 파라미터로 다시 말해 주는 구조는 층을 바꾸는 순간 조용히 어긋난다
# (floor2 250 / floor1 2740.2 → 2.49m 차이).
Z_NET = {"floor1": _Z1, "floor2": _Z2}
...
# 🚨 **층마다 다르다.** 위 실측은 전부 floor1 이야기인데, 한동안 이 값 하나를
#    두 코스가 같이 썼다. floor2 에 그대로 쓰면 s=585 는 **곡관 한복판**이다
#    (2026-08-10 계산, 사용자 보고 "첫 위치가 이상함"):
#        s    0.0 ~  185.0  직선(진입 라이저)  185mm  ← 로봇 188mm 가 안 들어간다
#        s  185.0 ~  445.5  곡관
#        s  445.5 ~  470.5  직선  25mm
#        s  470.5 ~  726.9  곡관              ← **0.585 가 여기다**
#        s  726.9 ~  956.1  직선 229mm        ← 온전히 들어가는 첫 구간
#    로봇이 s−156 ~ s+32 를 차지하므로 그 구간의 창은 **0.883~0.924(여유
#    41mm)**, 가운데가 0.90 이다. 곡관에서 출발하면 배치 코드가 로봇을
#    직선으로 눕히므로 뒤쪽 절반이 관벽을 파고든 채 깨어난다 — 실측 증상이
#    정확히 그것이었다: `안착 이탈 11.9mm`, 헤딩≠접선 경고, 3초 만에 끼임.
#    🔑 환경변수 `START_S` 를 주면 그 값이 이긴다(층 구분 없이).
START_S_BY_COURSE = {"floor1": 0.585, "floor2": 0.900}
_START_S_ENV = os.environ.get("START_S")


def start_s(course):
    """이 코스의 출발 호길이(m). 로봇을 **직선 구간에 온전히** 눕히는 값."""
    if _START_S_ENV:
        return float(_START_S_ENV)
    return START_S_BY_COURSE.get(course, 0.585)


START_S = start_s(RUN_NAMES[0])          # 진단 출력·기본값용
...
def build_robot(name, path_cl):
    ...
    _s0 = start_s(name)                  # 🔑 층마다 다르다 (START_S_BY_COURSE)
...
try:
    ...
    for r in robots:
        for nm, seg, x, fwd in CAM_SPECS:
            ...
            # 🌐 **단, `--ros` 면 후방도 만든다** (2026-08-09). 웹 관제가
            #    복귀·탈출 중에 **가는 방향**을 봐야 하기 때문이다 — 전방만
            #    내보내면 후진 화면이 지나온 쪽을 비춘다. "아무도 안 읽는다"는
            #    전제가 웹이 붙으면서 깨졌다. 주행만 볼 때(--ros 없음)는
            #    예전대로 안 만들어 렌더 비용을 아낀다.
            if NAV == "vision" and not fwd and not ROS:
                continue
            ...
            if fwd:
                ...
                # 🌐 후방은 **웹 화면 전용**이다 — 판정(detector)은 안 본다.
                #    깊이도 안 붙인다(검출이 전방만 쓴다, 규약도 front 만).
                r["cam_rear"] = cam
...
if NAV == "vision":
    ...
    for r in robots:
        ...
        # 🔑 노브를 들고 있는다 — GUI 재시작(Stop→Play)에서 컨트롤러를 **새로
        #    만들어야** 한다. 안 그러면 지난 주행의 적산거리·DONE 이 남아
        #    출발하자마자 복귀로 간다.
        r["ctl_kn"] = _ctl_kn
...
# 🔑 실전 맵은 **한 번에 한 층**이라 로봇도 한 대다 → 네임스페이스는 하나
#    (`ROS_NS`, 기본 `robot`). 로봇이 여럿이면 이름을 그대로 쓴다.
# 🚨 브리지는 자기 옆(`src/dongmin/pipe_comm`)의 규약을 직접 잡는다 — 여기서
#    pipe_comm 경로를 따로 세워 줄 필요가 없다.
bridge = None
if ROS:
    sys.path.append(str(SON.parent / "dongmin" / "isaac_bridge"))
    ...
    class _FrameAnn:
        """Isaac `Camera` 의 프레임을 annotator 인 척 넘긴다.

        브리지는 `get_data()` 하나만 쓴다. 🔑 **새 annotator 를 안 붙이는
        것**이 요점이다 — 판정(`condition/detector`)이 보는 바로 그 프레임을
        그대로 발행해야 화면과 로그를 대조할 수 있다(이식가이드 §1).
        """

        def __init__(self, cam, kind):
            self.cam, self.kind = cam, kind

        def get_data(self):
            try:
                if self.kind == "rgb":
                    a = self.cam.get_rgba()
                    # 첫 프레임 전에는 빈 1차원 배열이 온다 — 없는 것으로 친다
                    return a if getattr(a, "ndim", 0) == 3 else None
                return self.cam.get_current_frame().get("distance_to_camera")
            except Exception:
                return None

    _NS = [ROS_NS] if len(robots) == 1 else [r["name"] for r in robots]
    bridge = ros_bridge.Bridge(_NS)
    for _ns, r in zip(_NS, robots):
        r["pub"] = bridge.robot(_ns)
        r["ns"] = _ns
        # (1) 영상 — 전방(RGB+깊이)과 후방(RGB만). 🔑 **한 번에 한 대만**
        #     굽는다(아래 주 루프가 진행 방향으로 고른다) — 셋을 다 굽는 것은
        #     JPEG 인코딩 낭비다. 토치 카메라는 이 파일에 없다(용접 시퀀스가
        #     없으므로) → 웹의 torch 화면은 조용한 것이 정상이다.
        _cf = r.get("cam_front")
        if _cf is not None:
            r["pub"].use_annotators(_FrameAnn(_cf, "rgb"),
                                    _FrameAnn(_cf, "depth"),
                                    CAM_W, CAM_H, F_PX, "front")
        _cr = r.get("cam_rear")
        if _cr is not None:
            # 규약상 rear 토픽에는 깊이가 없다 → 깊이 annotator 는 안 넘긴다.
            r["pub"].use_annotators(_FrameAnn(_cr, "rgb"), None,
                                    CAM_W, CAM_H, F_PX, "rear")
        # (2) 코스 기하를 latched 로 한 번 선언 — web_panel 3D 맵의 단일 출처.
        #     중심선 표본이 촘촘하므로 20mm 간격으로 솎아서 보낸다.
        # 🚨 규약은 **`[s, x, y, z]`** 다 — 점 배열(N×3)을 그대로 넘기면
        #    `contract.course` 가 ValueError 로 즉시 죽는다(실측: 첫 판이
        #    `r["cl"].p` 를 그냥 넘겨 --ros 가 기동에서 세그폴트로 끝났다).
        #    s 는 호길이(m)이고 `Centerline` 이 이미 `cl.s` 로 들고 있다.
        _cl = r["cl"]
        _stride = max(1, int(round(
            0.020 / max(_cl.total / max(len(_cl.p) - 1, 1), 1e-9))))
        _crs = [[float(_cl.s[i]), *map(float, _cl.p[i])]
                for i in range(0, len(_cl.p), _stride)]
        if _crs[-1][0] < _cl.total - 1e-9:            # 끝점은 반드시 넣는다
            _crs.append([float(_cl.total), *map(float, _cl.p[-1])])
        # 🚨 z 오프셋을 같이 싣는다 — 층별 2대 동시 주행에서 관제 3D 맵이
        #    두 좌표계를 한 프레임으로 겹치는 데 쓴다(없으면 2.49m 어긋난다).
        r["pub"].publish_course(_crs, ir_m=PIPE_IR,
                                bend_r_m=_BEND_R / 1000.0,
                                z_shift_mm=-Z_NET[r["name"]])
        # (3) CAD 메시 — 없거나 낡았으면 여기서 굽는다(0.5초). 웹 PC 가 다른
        #     자리에 있어도 파일이 아니라 토픽으로 건너간다(latched 1회).
        #     z 오프셋은 활성 층 수평망을 월드 z=0 으로 올리는 값이다.
        r["pub"].publish_mesh(_MAP.with_suffix(".webmesh"), usd=_MAP,
                              z_shift_mm=-Z_NET[r["name"]])
    _ROS_EVERY = max(1, int(PHYSICS_HZ / 10))             # 10Hz
    # 연습장의 로봇 상태는 SETTLE→RUN 둘뿐이고, 자세한 국면은 컨트롤러가
    # 들고 있다(`r["ctl"].s.state`) — 규약 상태는 그쪽을 우선해서 좁힌다.
    _ST_CTL = {
        "IDLE": contract.STATE_HOLD, "HOLD": contract.STATE_HOLD,
        "CRUISE": contract.STATE_RUN, "SLOW": contract.STATE_RUN,
        "BRANCH_ENTRY": contract.STATE_RUN,
        "RECOVER": contract.STATE_STUCK,
        "RETURN": contract.STATE_RETURN,
        "DONE": contract.STATE_DONE,
    }
    _ST = {"SETTLE": contract.STATE_SETTLE, "RUN": contract.STATE_RUN}
    # 컨트롤러 상태가 바뀌는 순간 사건 하나 (이식가이드 §4). 🚨 10Hz 상태
    # 스트림으로 사건을 유추하면 샘플 사이의 전환이 통째로 사라진다.
    _EV_CTL = {"BRANCH_ENTRY": contract.EV_BRANCH,
               "RECOVER": contract.EV_STUCK,
               "RETURN": contract.EV_ARRIVE,
               "DONE": contract.EV_DONE}

    # ── 지령 (web_panel 의 Robot Handling 버튼이 여기로 온다) ──────
    # 🚨 **받는 것과 거는 것을 분리한다.** 콜백에서 로봇을 건드리면 물리 스텝
    #    중간에 상태가 바뀌어 재현이 안 된다 → 여기서는 플래그만 세우고, 주
    #    루프가 FSM 앞에서 건다. ESTOP 만 예외로 즉시 건다.
    # ⚠ 이 연습장에는 용접 시퀀스가 없다(점검 주행만). 그래서 dongyeon 시연의
    #   "용접 중에는 지령을 미룬다" 규칙이 여기서는 필요 없고, 지령이 곧바로
    #   걸린다. **본판에 옮길 때는 그 유예 규칙을 같이 가져갈 것.**
    SPEED_MIN_MPS = 0.005
    SPEED_MAX_MPS = CONTACT_OFFSET * PHYSICS_HZ / 1.2

    def _apply_mission(r, cmd):
        """지령 하나 → 플래그. 돌려주는 값은 로그 한 줄."""
        global TARGET_SPEED_MPS
        c = cmd.get("cmd")
        why = cmd.get("reason", "")
        _ctl = r.get("ctl")
        # ESTOP 해제는 START + reason 에 ESTOP_RELEASE 가 있을 때만. 실수로
        # ▶시작을 눌러 비상정지가 풀리면 안 되므로 지령을 새로 만들지 않는다.
        if r.get("estop") and c == contract.CMD_START \
                and "ESTOP_RELEASE" in why.upper():
            r["estop"], r["hold"] = False, True
            return "🔓 비상정지 해제 — 정지(HOLD)로 대기. ▶시작을 한 번 더"
        if r.get("estop"):
            return f"📥 {c} 무시 — 비상정지 상태다"
        if c == contract.CMD_START:
            r["hold"] = False
            return "▶ START — 주행 재개"
        if c == contract.CMD_STOP:
            r["hold"] = True
            return "⏸ STOP — 그 자리에 정지(재개 가능)"
        if c == contract.CMD_ESTOP:
            r["estop"], r["hold"] = True, True
            drive(r, 0.0)
            r["pub"].emit(contract.EV_ESTOP, why or "웹 지령")
            return "⛔ ESTOP — 그 자리에 언다. 재개하려면 해제가 필요하다"
        if c == contract.CMD_SPEED:
            try:
                v = float(cmd["mps"])
            except (KeyError, TypeError, ValueError):
                return f"📥 SPEED 인자가 없다 ({cmd!r})"
            v = min(max(v, SPEED_MIN_MPS), SPEED_MAX_MPS)
            # 🚨 **컨트롤러의 상한을 같이 갈아야** 한다. TARGET_SPEED_MPS 만
            #    바꾸면 vision 경로는 그 값을 안 보므로 화면 숫자만 변하고
            #    로봇은 그대로 간다(조용한 거짓말).
            TARGET_SPEED_MPS = v
            if _ctl is not None:
                _ctl.k["v_max_mps"] = v
            return f"⏩ SPEED {v * 1000:.0f}mm/s (상한 {SPEED_MAX_MPS * 1000:.0f})"
        if c == contract.CMD_RECALL:
            if r.get("recall"):
                return "↩ RECALL — 이미 복귀 중이다"
            r["recall"], r["hold"] = True, False
            # 🚨 `recall` 플래그를 **읽는 것은 vision 컨트롤러뿐**이었다
            #    (`ctl.step(recall=…)`). NAV=onboard/blueprint 로 돌리면
            #    아무도 안 읽어서 **버튼을 눌러도 아무 일이 없었다**(실측
            #    2026-08-10, 사용자 보고). 그쪽은 방향을 `r["dir"]` 로 도니
            #    여기서 직접 뒤집는다.
            if _ctl is None:
                r["dir"] = -1
            return "↩ RECALL — 방향을 뒤집어 배수구로 복귀한다"
        if c == contract.CMD_FORWARD:
            r["recall"], r["hold"] = False, False
            # 복귀 중 끼임 탈출용 임시 상태도 같이 지운다 — 안 지우면 다음
            # ↩ 복귀가 지난 판의 재개 지점을 물려받는다.
            r.pop("recall_resume_s", None)
            r["recall_tries"] = 0
            if _ctl is not None and _ctl.s.state == "RETURN":
                _ctl.s.state = "CRUISE"
            else:
                r["dir"] = +1          # onboard 판의 복귀 취소 (위와 짝)
            return "⏩ FORWARD — 복귀 취소, 다시 전진 점검"
        if c == contract.CMD_RETRY:
            if _ctl is None:
                return "↻ RETRY — vision 모드가 아니라 받을 수 없다"
            if _ctl.s.state == "RECOVER":
                return "↻ RETRY — 이미 탈출 시도 중이다"
            _ctl.s.state = "RECOVER"
            r["hold"] = False
            return "↻ RETRY — 후진 탈출 재시도"
        return f"📥 모르는 지령 {c!r} — 무시"

    print(f"[ROS] pipe_comm 규약 발행 — ns={', '.join(_NS)}, 카메라 "
          f"{len(robots[0]['pub'].cams)}대 "
          f"({', '.join(c[0] for c in robots[0]['pub'].cams) or '없음'})"
          + ("" if robots[0].get("cam_front") is not None
             else " — NAV=vision 이나 --cam 일 때만 영상이 나간다"))
    print("[ROS] 카메라 분기 — 전진 front / 후진(복귀·탈출) rear. "
          "토치 카메라는 이 파일에 없다(용접 시퀀스 없음) → 조용한 것이 정상")
...
while True:
    ...
    if not HEADLESS:
        ...
        if _playing and not was_playing:
            world.reset()
            # 🚨 **step 을 0 으로 되돌린다** (규약 §7). 웹은 step 이 뒤로 가는
            #    것으로 재시작을 알아채고 지나온 초록 궤적을 지운다 — 안
            #    되돌리면 이전 주행의 선이 그대로 남는다.
            step, _step_mark, _last_report = 0, 0, 0
            for r in robots:
                r.update(state="SETTLE", t=0, dir=+1, lap=0, stuck=0,
                         s_last=0.0, mark=step, dead=False, wheel_rad=0.0,
                         done=False, recall=False, started=False,
                         hold=WAIT_START, estop=False, s_hint=None,
                         s_prog=-1.0, prog_mark=step, ros_prev=None,
                         # 복귀 중 끼임 탈출용 임시 상태도 같이 되돌린다
                         recall_tries=0)
                r.pop("recall_resume_s", None)
                if r.get("ctl") is not None:
                    r["ctl"] = DriveController(r["ctl_kn"])
    ...
    # ── 규약 ② 매 스텝 spin + 지령 꺼내기 (이식가이드 §2·§5) ──────────
    # 🚨 **매 스텝** 부른다. 발행 주기에 묶으면 지령을 그만큼 늦게 받는다.
    #    timeout 0 이라 물리는 안 멈춘다.
    if bridge is not None:
        bridge.spin()
        if step % _ROS_EVERY == 0:                        # 10Hz 로 확인
            for r in robots:
                while True:
                    _cmd = r["pub"].pop_mission()
                    if _cmd is None:
                        break
                    print(f"[ROS] {_apply_mission(r, _cmd)}")
    ...
    for r in robots:
        ...
        # ── 규약 ⑤ 지령 적용 — FSM **앞**에서 건다 (이식가이드 §5) ──────
        # 🚨 컨트롤러를 **부르지 않고** 건너뛴다. 불러 두고 속도만 0 으로
        #    누르면 컨트롤러의 끼임 타이머가 계속 돌아 몇 초 뒤 스스로
        #    RECOVER 로 간다 — 사람이 세워 둔 것을 로봇이 끼임으로 오해한다.
        #    연습장의 조기 실패 판정(FAIL_S 제자리)도 같은 이유로 같이 멈춘다.
        # 🔑 **다리 예압은 계속 쓴다.** 안 쓰면 드라이브 타깃이 0 인 채로 남아
        #    다리가 벽을 안 밀고, 수직관에서는 그대로 흘러내린다.
        if r.get("estop") or r.get("hold"):
            drive(r, 0.0)
            r["v_cmd"] = 0.0
            r["mark"] = r["prog_mark"] = step
            r["s_last"] = r["s_prog"] = s_now
            if LEG_FORCE:
                _hi, _hv = force_legs(r)
                if _hi:
                    _set_eff(r, _hv, _hi)
            else:
                _hi, _hv = preload_legs(r)
                if _hi:
                    _set_pos(r, _hv, _hi)
            continue
        ...
        if off_now > PIPE_IR + 0.03 or not np.isfinite(p1).all():
            ...
            r["dead"] = True
            if bridge is not None:
                r["pub"].emit(contract.EV_OFF_COURSE,
                              f"중심선 이탈 {off_now * 1000:.0f}mm",
                              s_now * 1000.0)
            continue
        ...
        if NAV == "vision" and r.get("ctl") is not None:
            ...
            if abs(s_now - r.get("s_prog", -1.0)) > 0.003:
                ...
                r["dead"] = True
                if bridge is not None:
                    r["pub"].emit(contract.EV_STUCK,
                                  f"{FAIL_S:.0f}초 제자리 — 조기 중단",
                                  s_now * 1000.0)
                continue
            if r.get("ctl_state") == "DONE":
                ...
                drive(r, 0.0)
                # 🔑 이탈 사망과 **구분한다** — 규약 상태가 DEAD 냐 DONE 이냐가
                #    갈린다(웹은 빨간 경보 / 초록 완료로 달리 그린다).
                r["dead"], r["done"] = True, True
                if bridge is not None:
                    r["pub"].emit(contract.EV_HOME, "샤워 배수구로 복귀",
                                  s_now * 1000.0)
                    r["pub"].emit(contract.EV_DONE, "임무 완료",
                                  s_now * 1000.0)
                continue
        ...
        if adv > 0.003:
            ...
            r["dir"] *= -1
            r["s_last"], r["mark"] = s_now, step
            # 🔑 복귀 지시 중에 끼이면 앞으로 물러나 빠져나오는데, 그대로 두면
            #    **복귀가 거기서 흐지부지된다**(되돌릴 계기가 없다). 얼마나
            #    나아가면 다시 뒤로 돌지를 여기서 못박는다 — 컨트롤러 판의
            #    RECOVER→RETURN 복귀와 같은 뜻이다.
            if r.get("recall") and r["dir"] > 0:
                r["recall_resume_s"] = s_now + 0.06
                r["recall_tries"] = r.get("recall_tries", 0) + 1
                # 🚨 진입 라이저(floor2 335mm)를 **거꾸로 올라가는 것**이 복귀의
                #    제일 어려운 구간이다(기록된 실패). 같은 자리에서 세 번
                #    걸리면 사람이 알아야 한다 — 자동으로 접지는 않는다
                #    (⏩ 전진으로 복귀를 취소할지는 조작하는 쪽이 정한다).
                if r["recall_tries"] == 3:
                    print(f"[{r['name']:8s}] ⚠ 복귀 중 끼임 3회 "
                          f"(s={s_now * 1000:.0f}mm) — 라이저 등반에서 막히는 "
                          f"중일 수 있다. ⏩ 전진으로 복귀를 접을 수 있다")
        ...
        # 끼임 탈출로 앞으로 물러났다가, 충분히 나왔으면 복귀를 이어 간다
        if r.get("recall") and r["dir"] > 0 \
                and s_now >= r.get("recall_resume_s", 1e9):
            r["dir"] = -1
            r["s_last"], r["mark"] = s_now, step
            r.pop("recall_resume_s", None)
            print(f"[{r['name']:8s}] ↩ 복귀 재개 (s={s_now * 1000:.0f}mm)")
        ...
        if r["dir"] > 0 and s_now >= r["cl"].total - END_S:
            ...
            # 🚨 **`i0` 는 배수구가 아니라 "로봇을 놓은 자리"다** (START_S,
            #    floor2 에서 s≈595mm). 왕복 주행은 여기서 되돌리는 게 맞지만
            #    ↩ 복귀는 아니다 — 복귀의 끝은 **샤워 배수구 밖**(EXIT_S)이다.
            #    처음 고칠 때 이 둘을 같은 것으로 보아, 복귀를 누르자마자
            #    (이미 출발점 뒤였다) 곧바로 "임무 완료" 가 났다(실측 2026-08-10:
            #    s=588mm 에서 즉시 종료). 복귀 중에는 여기서 되돌리지 않고
            #    라이저를 타고 계속 내려간다 — 완료 판정은 아래 EXIT_S 다.
            if not r.get("recall"):
                r["lap"] += 1
                r["dir"] = +1
                r["s_last"], r["mark"] = s_now, step
        ...
        # ↩ 복귀 완료 = **배수구 밖**. vision 판의 EXIT_S 판정과 같은 기준이다
        #   (거기서는 컨트롤러의 DONE 뒤에 위치로 한 번 더 확인한다).
        if r.get("recall") and s_now <= EXIT_S:
            print(f"[{r['name']:8s}] ✅ **임무 완료** — 샤워 배수구로 복귀 "
                  f"(s={s_now * 1000:.0f}mm, 왕복 {r['lap']})")
            drive(r, 0.0)
            # 이탈 사망과 구분한다 — 웹이 빨간 경보/초록 완료로 달리 그린다
            r["dead"], r["done"] = True, True
            if bridge is not None:
                r["pub"].emit(contract.EV_HOME, "지시 복귀 완료",
                              s_now * 1000.0)
                r["pub"].emit(contract.EV_DONE, "임무 완료", s_now * 1000.0)
            continue

    # ── 규약 ③④ 발행 — web_panel 이 보는 유일한 창구 (10Hz) ────────────
    # 🚨 **종료 검사보다 앞에 둔다.** 뒤에 두면 임무가 끝나는 순간 발행도
    #    지령 수신도 통째로 멈춰 화면이 얼어붙는다(이식가이드 §3 실측 교훈).
    if bridge is not None and step % _ROS_EVERY == 0:
        for r in robots:
            _p1 = wpos(r["seg1"])
            _s, _off, _ = r["cl"].nearest(_p1, r.get("s_hint"))
            _cs = r.get("ctl_state")
            _held = bool(r.get("estop") or r.get("hold"))
            # 🚨 **부호는 두 군데에서 온다.** vision 은 `v_cmd` 에 부호를 싣고
            #    (`dir` 은 늘 +1), onboard/blueprint 는 `v_cmd` 를 크기로만
            #    쓰고 방향을 `dir` 로 뒤집는다. `v_cmd` 만 보면 onboard 에서는
            #    **후진 중에도 direction=+1 · cam=front** 로 나간다 — 실측
            #    2026-08-10: 끼임 탈출·복귀로 뒤로 가는데 웹은 전방 카메라를
            #    계속 보여 줬다. 둘을 곱해 하나의 부호로 만든다.
            _vc = float(r.get("v_cmd", 0.0)) \
                * (-1.0 if r.get("dir", 1) < 0 else 1.0)
            if r.get("estop") or (r["dead"] and not r.get("done")):
                _st = contract.STATE_DEAD
            elif r.get("done"):
                _st = contract.STATE_DONE
            elif _held:
                _st = contract.STATE_HOLD
            elif r["state"] == "SETTLE":
                _st = contract.STATE_SETTLE
            else:
                _st = _ST_CTL.get(_cs, _ST.get(r["state"], contract.STATE_RUN))
            # 🔑 **서 있는 동안 상태를 그렇게 싣는다.** CRUISE 를 그대로 내면
            #    화면은 "주행 중" 인데 실제로는 멈춰 있다. 뭘 하다 멈췄는지는
            #    reason 에 남긴다.
            _rs = _cs or r["state"]
            if _held:
                _rs = f"{'ESTOP' if r.get('estop') else 'HOLD'}({_rs})"
            # 🔑 **카메라는 가는 방향을 본다** — 복귀·탈출은 음수 지령이므로
            #    그때 후방으로 갈아 끼운다(이식가이드 §3 "전진 front / 후진
            #    rear"). 🚨 **서 있는 동안에는 마지막 것을 유지한다** — v=0
            #    이라고 전방으로 되돌리면 복귀 중 정지에서 화면이 앞뒤로
            #    튄다. 후방이 없으면(--ros 없이 만든 씬) 전방으로 둔다.
            if abs(_vc) > 1e-6:
                r["cam_role"] = "rear" if _vc < 0 else "front"
            _cam = r.get("cam_role", "front")
            if r.get("cam_rear") is None:
                _cam = "front"
            # 🚨 speed 는 항상 **양수**, 방향은 direction 으로. 복귀는
            #    컨트롤러가 음수 지령으로 내므로 부호를 여기서 갈라 싣는다.
            r["pub"].publish_state(
                state=_st,
                direction=(-1 if _vc < 0 else 1),
                speed_mps=(0.0 if _held else abs(_vc)),
                s_mm=_s * 1000.0, s_total_mm=r["cl"].total * 1000.0,
                off_mm=_off * 1000.0, lap=r["lap"], stuck=r["stuck"],
                step=step, roll_deg=r.get("roll_abs", 0.0), reason=_rs,
                art=r.get("art"), wheel_idx=r.get("wheel"), pos=_p1,
                cam=_cam)
            # 영상 — **지금 쓰는 한 대만** 굽는다. 안 켠 카메라 토픽은
            # 정상적으로 조용하고, 웹은 위 `cam` 필드로 어느 것인지 안다.
            if r.get("cam_front") is not None:
                r["pub"].publish_camera(only=_cam)
            # ── 규약 ④ 상태가 바뀌는 순간 사건 하나 ──────────────
            _key = _cs or r["state"]
            if _key != r.get("ros_prev"):
                _ev = (contract.EV_START
                       if r.get("ros_prev") in (None, "SETTLE")
                       and _key != "SETTLE" else _EV_CTL.get(_key))
                if _ev:
                    r["pub"].emit(_ev, f"{r.get('ros_prev') or '-'} → {_key}",
                                  _s * 1000.0)
                r["ros_prev"] = _key
...
for r in robots:
    ...
    print(f"결과  {r['name']:8s} 왕복 {r['lap']}회  끼임 {r['stuck']}회  "
          ...
          # 🚨 `dead` 는 "더 안 움직인다" 는 뜻이고 이유가 둘이다 — 이탈 사고와
          #    임무 완료. 예전에는 둘 다 "❌ 이탈로 정지" 로 찍혀서, 복귀를
          #    제대로 마친 판까지 실패로 읽혔다(실측 2026-08-10).
          + ("  ✅ 임무 완료" if r.get("done")
             else "  ❌ 이탈로 정지" if r["dead"] else ""))
if bridge is not None:
    bridge.shutdown()
sys.stdout.flush()
